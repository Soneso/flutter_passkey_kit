...
class AuthService {
  ...
  static Future<PublicKeyCredential> createPasskeyCredentials(
    ...
    final savedCredentials = await credentialManager
        .savePasskeyCredentials(request: cd.CredentialCreationOptions.fromJson(options.toJson()));
  ...
  static Future<PublicKeyCredential> getPasskeyCredentials(
    ...
    if (credResponse.publicKeyCredential == null) {
      throw Exception('Invalid passkey login response: publicKeyCredential not found');
    }

    return PublicKeyCredential.fromJson(credResponse.publicKeyCredential!.toJson());
  }
}
